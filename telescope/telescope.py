  ...
  def retrieve_data_upon_job_completion(self, job_id, query_object=None):
    ...
    if query_object:
      try:
        ...
        logger.debug('Received data, processing according to %s metric.',
        ...
        logger.error(
            ('Caught {caught_error} for ({site}, {client_provider}, {metric}, '
             '{date}).').format(caught_error=caught_error, **self._metadata))
  ...
  try:
    with open(data_filepath, 'w') as data_file_raw:
      if metric_calculations:
        data_file_csv = csv.DictWriter(
            data_file_raw, fieldnames=metric_calculations[0].keys(),
    ...
    if caught_error.errno == 24:
      logger.error('When writing raw output, caught %s, trying again shortly.',
                   caught_error)
      write_metric_calculations_to_file(data_filepath, metric_calculations,
                                        should_write_header)
      time.sleep(20)
    else:
...
def generate_query(selector, ip_translator, mlab_site_resolver):
  ...
  if selector.site:
    try:
      retrieved_site_ips = mlab_site_resolver.get_site_ndt_ips(selector.site)
      for retrieved_site_ip in retrieved_site_ips:
        server_ips.append(retrieved_site_ip)
        logger.debug('Found IP for %s of %s.', selector.site, retrieved_site_ip)
    except Exception as caught_error:
...
def process_selector_queue(selector_queue, google_auth_config,
  ...
  while not selector_queue.empty():
    ...
    if batchmode == 'all':
      ...
      logger.info(('Found %d tables, when the maximum for non-batched mode is '
                   '%d. Setting query to batched mode (this will increase the '
                   'amount of time but lower failure rate). This behavior can '
                   'be controled with the --batchmode argument.'),
                  bq_table_span, max_tables_without_batch)
      is_batched_query = True
    else:
    ...
    external_query_handler = ExternalQueryHandler(data_filepath,
                                                  thread_metadata)
    external_query_handler.queue_set = (bq_query_string,
                                        bq_table_span,
                                        thread_metadata,
                                        data_filepath,
                                        True)
  ...
  for selector in selectors:
    ...
    if not args.ignorecache and utils.check_for_valid_cache(data_filepath):
      logger.info(('Raw data file found (%s), assuming this is '
                   ...
                   '--ignorecache to suppress this behavior.'), data_filepath)
      continue

    logger.debug('Did not find existing data file: %s', data_filepath)
    logger.debug(('Generating Query for subset of {site}, {client_provider}, '
    ...
    try:
      ...
      bq_query_string, bq_table_span = generate_query(selector, ip_translator,
                                                      mlab_site_resolver)
    ...
    if not args.dryrun:
      ...
      logger.warn('Dry run flag caught, built query and reached the point that '
...
if __name__ == '__main__':
  ...
  parser.add_argument('--maxminddir', default='resources/',
  ...
  parser.add_argument('--noauth_local_webserver', default=False,
                      ...
                      help=('Authenticate to Google using another method than a'
                            ' local webserver.'))
  parser.add_argument('--batchmode', default='automatic',
                      choices=['all', 'automatic', 'none'],
                      help='Control how batch mode is used to query BigQuery.')
  parser.add_argument('--credentialspath', dest='credentials_filepath',
